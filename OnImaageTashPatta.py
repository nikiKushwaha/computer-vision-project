...
def load_input():
    ...
    augment_image = cv2.resize(augment_imaage, (300,400))
...
def compute_matches(descriptors_input, descriptors_output):
    ...
    if(len(descriptors_output)!=0 and len(descriptors_input)!=0):
        matches = flann.knnMaatch(np.asarray(descriptors_input,np.float32),np.asarray(descriptors_output,np.float32),k=2)
        good = []
        for m,n in matches:
...
if __name__ =='__main__':
    ...
    while(ret):
         ...
         if(matches!=None):
            if(len(matches)>10):
              src_pts = np.float32([ input_keypoints[m.queryIdx].pt for m in matched]).reshape(-1,1,2)
              dst_pts = np.float32([ output_keypoints[m.trainIdx].pt for m in matched]).reshape(-1,1,2)
              ...
              #GETTING THE frame ready for adition operation with Mask Image
              frameb = cv2.fillConvexPoly(frame,dst.astype(int),0) 
              Final = frameb+M_aug

              #output final = cv2.polylines(frame,[np.int32(dst)],True,255,3)
